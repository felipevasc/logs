...
const files = new Map(), MAX_FILE = 64 * 1024 * 1024, MAX_BODY = Math.ceil(MAX_FILE * 4 / 3) + 8192, MAX_CACHE = MAX_FILE * 2;
...
export async function handlePreviewDownload(req, res, url) {
  ...
  if (req.method === "POST" && url.pathname === prefix) {
    ...
    if (!["png", "pdf", "json"].includes(format) || !filename.endsWith(`.${format}`) || filename.length > 190 || /[\\/\x00-\x1f\x7f]/.test(filename) || typeof value.base64 !== "string" || !/^[A-Za-z0-9+/]+={0,2}$/.test(value.base64)) { json(res, 400, { error: "Nome ou formato inválido." }); return true; }
    const bytes = Buffer.from(value.base64, "base64");
    let valid = format === "png" ? bytes.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])) : bytes.subarray(0, 5).toString() === "%PDF-";
    if (format === "json") { try { const parsed = JSON.parse(bytes.toString("utf8")); valid = parsed && typeof parsed === "object" && !Array.isArray(parsed); } catch { valid = false; } }
    if (!valid || bytes.length > (format === "json" ? MAX_FILE : 32 * 1024 * 1024)) { json(res, 400, { error: "Arquivo inválido." }); return true; }
  ...
  if (req.method === "GET" || req.method === "HEAD") {
    ...
    res.writeHead(200, { "content-type": file.format === "pdf" ? "application/pdf" : file.format === "json" ? "application/json; charset=utf-8" : "image/png", "content-length": file.bytes.length,
